...
| 23 to 38 seconds | Point to Passport #1 fields: vault, runtime code hash, asset, 0.10 aUSDC cap, 24-hour window, and nonce 0. | "The passport binds authority to this deployed bytecode, this asset, this cap, this expiry, and this nonce. A controller change, code change, expired mandate, cap breach, or failed Cleanverse check returns a deterministic reason and blocks the transfer." | Concept and Problem Definition, 20. Build Quality, 25. | Passport #1 was issued onchain. The same fields and fail-closed branches are covered by the contract tests. |
...
| 70 to 82 seconds | Return to the workspace, then briefly show the successful test command output or repository test section. | "The same enforcement is tested against bad recipients, cap breaches, expiry, revocation, controller and code changes, and Validator failure. Cleanverse credentials stay on the server, while the judge path needs no wallet or secret." | Build Quality, 25. UX and Demo, 15. | Foundry and web suites are repeatable from the public repository. The workspace is read-only and its evidence links are public. |
