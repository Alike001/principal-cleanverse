...
describe("principal passport snapshot", () => {
  it("fails closed while the Validator pool is not registered", () => {
    ...
    expect(principalPassport.statusDetail).toContain("cannot issue a passport or attempt a CVA transfer");
  ...
  it("keeps the known scope narrow and public evidence redacted", () => {
    expect(principalPassport.authority).toBe("One aUSDC transfer capability, pending");
